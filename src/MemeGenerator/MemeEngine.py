    ...
    def make_meme(self, img_path, text, author, width=500):
        ...
        try:
            ...
            if image.size[0] != width:
                width_percents = (width / float(image.size[0]))
                height = int((float(image.size[1]) * float(width_percents)))
                image.thumbnail((width, height), Image.LANCZOS)

            im_draw = ImageDraw.Draw(image)
            im_font = ImageFont.truetype('MemeGenerator/font/AlexBrush-Regular.ttf', 40)
            words = text.split()
            grouped_word = [' '.join(words[i: i + 8]) for i in range(0, len(words), 8)]
            position = 10
            for word in grouped_word:
